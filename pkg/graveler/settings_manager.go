...
// SettingsManager is a key-value store for Graveler repository-level settings.
...
type SettingsManagerOption func(m *SettingsManager)

func WithCache(cache cache.Cache) SettingsManagerOption {
	return func(m *SettingsManager) {
		m.WithCache(cache)
	}
}

func (m *SettingsManager) WithCache(cache cache.Cache) {
	m.cache = cache
}

func NewSettingsManager(refManager RefManager, store kv.Store, options ...SettingsManagerOption) *SettingsManager {
	m := &SettingsManager{
		refManager: refManager,
		store:      store,
...
func (m *SettingsManager) Save(ctx context.Context, repository *RepositoryRecord, key string, setting proto.Message, lastKnownChecksum *string) error {
...
func (m *SettingsManager) GetLatest(ctx context.Context, repository *RepositoryRecord, key string, dst proto.Message) (*string, error) {
	settings, err := m.store.Get(ctx, []byte(RepoPartition(repository)), []byte(SettingsPath(key)))
	if errors.Is(err, kv.ErrNotFound) {
...
func (m *SettingsManager) Get(ctx context.Context, repository *RepositoryRecord, key string, dst proto.Message) error {
	k := cacheKey{
...
func logSetting(logger logging.Logger, repositoryID RepositoryID, key string, setting proto.Message, logMsg string) {
	if logger.IsTracing() {
		logger.
