...
func newDiffTestRange(p *graveler.RangeDiff) *diffTestRange {
...
func TestDiff(t *testing.T) {
	...
	for name, tst := range tests {
		t.Run(name, func(t *testing.T) {
			...
			it := graveler.NewDiffIterator(ctx, fakeLeft, fakeRight)
			defer it.Close()
...
func TestDiffCancelContext(t *testing.T) {
	...
	cancel()
	it := graveler.NewDiffValueIterator(ctx, left, right)
	defer it.Close()
	if it.Next() {
...
func TestNextRange(t *testing.T) {
	ctx := context.Background()
	it := graveler.NewDiffIterator(ctx,
	...
	t.Run("call next range with no range", func(t *testing.T) {
		...
		if err := it.Err(); !errors.Is(err, graveler.ErrNoRange) {
			t.Fatalf("expected to get err=%s, got: %s", graveler.ErrNoRange, err)
		}
	})
}
...
func TestDiffSeek(t *testing.T) {
	...
	ctx := context.Background()
	it := graveler.NewDiffIterator(ctx, newFakeMetaRangeIterator(left, leftIdentities), newFakeMetaRangeIterator(right, rightIdentities))
	defer it.Close()
...
func TestNextOnClose(t *testing.T) {
	ctx := context.Background()
	it := graveler.NewDiffIterator(ctx, newFakeMetaRangeIterator([][]string{{"k1", "k2"}}, [][]string{{"i1", "i2"}}), newFakeMetaRangeIterator([][]string{{"k1", "k2"}}, [][]string{{"i1a", "i2a"}}))
	if !it.Next() {
		t.Fatal("expected iterator to have value")
	}
	it.Close()
...
func TestDiffErr(t *testing.T) {
	...
	ctx := context.Background()
	it := graveler.NewDiffIterator(ctx, leftIt, rightIt)
	defer it.Close()
	if it.Next() {
...
func newFakeMetaRangeIterator(rangeKeys [][]string, rangeIdentities [][]string) *graveler.FakeIterator {
	res := graveler.NewFakeIterator()
	for rangeIdx, keys := range rangeKeys {
		...
		res.
			AddRange(&graveler.Range{ID: graveler.ID(strings.Join(identities, "-")), MinKey: minKey, MaxKey: maxKey, Count: int64(len(rangeKeys[rangeIdx]))}).
