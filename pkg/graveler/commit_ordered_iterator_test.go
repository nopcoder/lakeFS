...
func TestOrderedCommitIterator(t *testing.T) {
	...
	for _, tst := range cases {
		t.Run(tst.Name, func(t *testing.T) {
			...
			iterator, err := graveler.NewOrderedCommitIterator(ctx, store, repository, false)
			if err != nil {
			...
			iterator.Close()
			iterator, err = graveler.NewOrderedCommitIterator(ctx, store, repository, true)
			if err != nil {
				t.Fatal("create ordered commit iterator", err)
			}
...
func TestOrderedCommitIteratorGrid(t *testing.T) {
	...
	iterator, err := graveler.NewOrderedCommitIterator(ctx, store, &repo, false)
	if err != nil {
		t.Fatal("create ordered commit iterator", err)
	}
	var actualOrderedCommits []string
	...
	iterator.Close()
	iterator, err = graveler.NewOrderedCommitIterator(ctx, store, &repo, true)
	if err != nil {
		t.Fatal("create ordered commit iterator", err)
	}
	var actualAncestryLeaves []string
...
func TestOrderedCommitIterator_CloseTwice(t *testing.T) {
	...
	it, err := graveler.NewOrderedCommitIterator(ctx, store, repository, false)
	if err != nil {
