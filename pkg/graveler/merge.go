...
func (m *merger) getNextGEKey(key Key) (*ValueRecord, error) {
...
// writeRecord writes ValueRecord using writer
func (m *merger) writeRecord(writeValue *ValueRecord) error {
	if m.logger.IsTracing() {
		m.logger.WithFields(logging.Fields{
...
func (m *merger) destBeforeSource(destValue *ValueRecord) error {
	...
	if baseValue != nil && bytes.Equal(destValue.Identity, baseValue.Identity) && bytes.Equal(destValue.Key, baseValue.Key) { // source deleted this record
		...
		if baseValue != nil && bytes.Equal(destValue.Key, baseValue.Key) { // deleted by source changed by dest
			switch m.strategy {
			case MergeStrategyDest:
				break
			case MergeStrategySrc:
				m.haveDest = m.dest.Next()
				return nil
...
func (m *merger) sourceBeforeDest(sourceValue *ValueRecord) error {
...
// receives a MergeStrategy parameter - strategyToInclude - to indicate
...
func (m *merger) handleAll(iter Iterator, strategyToInclude MergeStrategy) error {
	for {
		select {
		case <-m.ctx.Done():
			return m.ctx.Err()
		...
		if iterValue == nil {
			...
			if baseValue == nil || !bytes.Equal(baseValue.Identity, iterValue.Identity) {
				...
				if baseValue != nil && bytes.Equal(baseValue.Key, iterValue.Key) { // deleted by one changed by iter
					if m.strategy == MergeStrategyNone { // conflict is only reported if no strategy is selected
						return ErrConflictFound
					}
...
func (m *merger) handleConflict(sourceValue *ValueRecord, destValue *ValueRecord) error {
	switch m.strategy {
	case MergeStrategyDest:
		err := m.writeRecord(destValue)
		if err != nil {
...
func (m *merger) handleBothKeys(sourceValue *ValueRecord, destValue *ValueRecord) error {
...
func (m *merger) handleDestRangeSourceKey(destRange *Range, sourceValue *ValueRecord) error {
...
func (m *merger) handleSourceRangeDestKey(sourceRange *Range, destValue *ValueRecord) error {
...
func (m *merger) merge() error {
	...
	if m.haveSource {
		if err := m.handleAll(m.source, MergeStrategySrc); err != nil {
			return err
		}
	}
	if m.haveDest {
		if err := m.handleAll(m.dest, MergeStrategyDest); err != nil {
			return err
		}
	}
	return nil
...
func Merge(ctx context.Context, writer MetaRangeWriter, base Iterator, source Iterator, destination Iterator, strategy MergeStrategy) error {
	m := merger{
		ctx:      ctx,
