...
type commitsPriorityQueue []*CommitRecord
...
func NewCommitIterator(ctx context.Context, config *CommitIteratorConfig) *CommitIterator {
	return &CommitIterator{
		...
		visit:       make(map[CommitID]struct{}),
...
func (ci *CommitIterator) getCommitRecord(commitID CommitID) (*CommitRecord, error) {
...
func (ci *CommitIterator) Next() bool {
	...
	ci.value = heap.Pop(&ci.queue).(*CommitRecord)
...
//
//	The list of commit
func (ci *CommitIterator) SeekGE(id CommitID) {
	ci.err = nil
	ci.queue = make(commitsPriorityQueue, 0)
	ci.visit = make(map[CommitID]struct{})
...
func (ci *CommitIterator) Value() *CommitRecord {
	return ci.value
}
