	...
	tests := []struct {
	...
	}{
		{
			...
			sourceRange: newTestMetaRange([]testRange{
				{
					rng: graveler.Range{ID: "a/1-a/2", MinKey: graveler.Key("a/1"), MaxKey: graveler.Key("a/2"), Count: 2, EstimatedSize: 1024},
					records: []testValueRecord{
						{"a/1", "a1"}, {"a/2", "a2"},
					},
				},
				{
					rng: graveler.Range{ID: "a/3-a/6", MinKey: graveler.Key("a/3"), MaxKey: graveler.Key("a/6"), Count: 4, EstimatedSize: 2048},
					records: []testValueRecord{
			...
			destRange: newTestMetaRange([]testRange{
				{
					rng: graveler.Range{ID: "a/7-a/9", MinKey: graveler.Key("a/7"), MaxKey: graveler.Key("a/9"), Count: 3, EstimatedSize: 1536},
					records: []testValueRecord{
		...
		},
		{
			...
			sourceRange: newTestMetaRange([]testRange{
				{
					rng: graveler.Range{ID: "b/1-b/2", MinKey: graveler.Key("b/1"), MaxKey: graveler.Key("b/2"), Count: 2, EstimatedSize: 1024},
					records: []testValueRecord{
						{"b/1", "b1"}, {"b/2", "b2"},
					},
				},
		...
		},
		{
			...
			destRange: newTestMetaRange([]testRange{
				...
				{
					rng: graveler.Range{ID: "c/4-c/7", MinKey: graveler.Key("c/4"), MaxKey: graveler.Key("c/7"), Count: 2, EstimatedSize: 1024},
					records: []testValueRecord{
						{"c/4", "c4"}, {"c/7", "c7"},
					},
				},
		...
		},
		{
			...
			sourceRange: newTestMetaRange([]testRange{
				{
					rng: graveler.Range{ID: "a/4-a/8", MinKey: graveler.Key("a/4"), MaxKey: graveler.Key("a/8"), Count: 5, EstimatedSize: 2560},
					records: []testValueRecord{
			...
			destRange: newTestMetaRange([]testRange{
				{
					rng: graveler.Range{ID: "a/1-a/6", MinKey: graveler.Key("a/1"), MaxKey: graveler.Key("a/6"), Count: 6, EstimatedSize: 3072},
					records: []testValueRecord{
						{"a/1", "a1"}, {"a/2", "a2"}, {"a/3", "a3"}, {"a/4", "a4"}, {"a/5", "a5"}, {"a/6", "a6"},
					},
				},
				{
					rng: graveler.Range{ID: "b/1-b/6", MinKey: graveler.Key("b/1"), MaxKey: graveler.Key("b/6"), Count: 2, EstimatedSize: 1024},
					records: []testValueRecord{
		...
		},
		{
			...
			destRange: newTestMetaRange([]testRange{
				...
				{
					rng: graveler.Range{ID: "a/11-a/16", MinKey: graveler.Key("a/11"), MaxKey: graveler.Key("a/16"), Count: 6, EstimatedSize: 3072},
					records: []testValueRecord{
						{"a/11", "a11"}, {"a/12", "a12"}, {"a/13", "a13"}, {"a/14", "a14"}, {"a/15", "a15"}, {"a/16", "a16"},
					},
				},
				{
					rng: graveler.Range{ID: "a/21-a/26", MinKey: graveler.Key("a/21"), MaxKey: graveler.Key("a/26"), Count: 6, EstimatedSize: 3072},
					records: []testValueRecord{
		...
		},
		{
			...
			sourceRange: newTestMetaRange([]testRange{
				{
					rng: graveler.Range{ID: "a/1-a/4", MinKey: graveler.Key("a/1"), MaxKey: graveler.Key("a/4"), Count: 4, EstimatedSize: 2048},
					records: []testValueRecord{
						{"a/1", "a1"}, {"a/2", "a2"}, {"a/3", "a3"}, {"a/4", "a4"},
					},
				},
				{
					rng: graveler.Range{ID: "a/8-c/3", MinKey: graveler.Key("a/8"), MaxKey: graveler.Key("c/3"), Count: 2, EstimatedSize: 1024},
					records: []testValueRecord{
						{"a/8", "a8"}, {"c/3", "c3"},
					},
				},
			...
			destRange: newTestMetaRange([]testRange{
				{
					rng: graveler.Range{ID: "a/2-a/5", MinKey: graveler.Key("a/2"), MaxKey: graveler.Key("a/5"), Count: 4, EstimatedSize: 2048},
					records: []testValueRecord{
						{"a/2", "a2"}, {"a/3", "a3"}, {"a/4", "a4"}, {"a/5", "a5"},
					},
				},
				{
					rng: graveler.Range{ID: "a/12-a/15", MinKey: graveler.Key("a/12"), MaxKey: graveler.Key("a/15"), Count: 4, EstimatedSize: 2048},
					records: []testValueRecord{
						{"a/12", "a12"}, {"a/13", "a13"}, {"a/14", "a14"}, {"a/15", "a15"},
					},
				},
				{
					rng: graveler.Range{ID: "a/8-b/4", MinKey: graveler.Key("a/8"), MaxKey: graveler.Key("b/4"), Count: 4, EstimatedSize: 2048},
					records: []testValueRecord{
						{"a/8", "a8"}, {"b/1", "b1"}, {"b/2", "b2"}, {"b/4", "b4"},
					},
				},
				{
					rng: graveler.Range{ID: "b/6-b/7", MinKey: graveler.Key("b/6"), MaxKey: graveler.Key("b/7"), Count: 2, EstimatedSize: 1024},
					records: []testValueRecord{
						{"b/6", "b6"}, {"b/7", "b7"},
					},
				},
				{
					rng: graveler.Range{ID: "b/8-c/2", MinKey: graveler.Key("b/8"), MaxKey: graveler.Key("c/2"), Count: 4, EstimatedSize: 2048},
					records: []testValueRecord{
						{"b/8", "b8"}, {"b/9", "b9"}, {"c/1", "c1"}, {"c/2", "c2"},
					},
				},
				{
					rng: graveler.Range{ID: "c/5-d/2", MinKey: graveler.Key("c/5"), MaxKey: graveler.Key("d/2"), Count: 4, EstimatedSize: 2048},
					records: []testValueRecord{
	...
	for _, tst := range tests {
		for _, expectedResult := range tst.expectedResult {
			t.Run(tst.name, func(t *testing.T) {
				...
				metaRangeManager.EXPECT().NewMetaRangeIterator(gomock.Any(), gomock.Any(), graveler.MetaRangeID("")).AnyTimes().Return(graveler.NewEmptyIterator(), nil) // empty base
				...
				rangeManagers := make(map[graveler.StorageID]graveler.RangeManager)
				rangeManagers[tst.storageID] = rangeManager
				metaRangeManagers := make(map[graveler.StorageID]graveler.MetaRangeManager)
				metaRangeManagers[tst.storageID] = metaRangeManager
				committedManager := graveler.NewCommittedManager(metaRangeManagers, rangeManagers, params)
