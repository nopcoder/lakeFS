...
func TestCompare(t *testing.T) {
	...
	for name, tst := range tests {
		t.Run(name, func(t *testing.T) {
			diffIt := graveler.NewDiffIter(tst.diffs)
			defer diffIt.Close()
			base := makeBaseIterator(tst.baseKeys)

			ctx := context.Background()
			it := graveler.NewCompareIterator(ctx, graveler.NewDiffIteratorWrapper(diffIt), base)
			defer it.Close()
...
func TestCompareSeek(t *testing.T) {
	...
	diffIt := graveler.NewDiffIter(diffs)
...
func makeBaseIterator(keys []string) *graveler.FakeIterator {
	base := graveler.NewFakeIterator()
	if len(keys) == 0 {
		return base
	}
