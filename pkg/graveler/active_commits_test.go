...
func (c *fakeRepositoryCommitGetter) List(_ context.Context) (graveler.CommitIterator, error) {
	return graveler.NewFakeCommitIterator(c.Commits), nil
}
...
func TestCommitsMap(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.Name, func(t *testing.T) {
			commitsMap, err := graveler.NewCommitsMap(context.Background(), tc.CommitGetter)
			if err != nil {
				t.Fatal(err)
			}
...
func TestActiveCommits(t *testing.T) {
	...
	for name, tst := range tests {
		t.Run(name, func(t *testing.T) {
			...
			refManagerMock := graveler.NewMockRefManager(ctrl)
			...
			refManagerMock.EXPECT().ListCommits(ctx, repositoryRecord).Return(graveler.NewFakeCommitIterator(commitsRecords), nil).MaxTimes(1)

			gcCommits, err := graveler.GetGarbageCollectionCommits(ctx, graveler.NewGCStartingPointIterator(
				graveler.NewFakeCommitIterator(findMainAncestryLeaves(now, tst.headsRetentionDays, tst.commits)),
				graveler.NewFakeBranchIterator(branches)), &repositoryCommitGetter{
