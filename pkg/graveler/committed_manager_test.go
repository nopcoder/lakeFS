...
func TestManager_WriteRange(t *testing.T) {
	...
	writeResult := &graveler.WriteResult{
		RangeID:                 "someid",
		First:                   graveler.Key("1"),
		Last:                    graveler.Key("10"),
		Count:                   2,
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			sut := graveler.NewCommittedManager(metaRangeManagers, rangeManagers, params)

			times := 0
			expectedTimes := min(len(tt.records), maxRecords)
			if tt.errorIs == nil {
				rangeWriter.EXPECT().Abort().Return(nil)
				rangeManager.EXPECT().GetWriter(context.Background(), graveler.Namespace(ns), nil).Return(rangeWriter, nil)
				...
				rangeWriter.EXPECT().SetMetadata(graveler.MetadataTypeKey, graveler.MetadataRangesType)
			}

			it := graveler.NewFakeValueIterator(tt.records)
			rangeInfo, err := sut.WriteRange(context.Background(), tt.writeStorageID, graveler.StorageNamespace(ns), it)
			if tt.errorIs != nil {
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.errorIs == nil {
				...
				metarangeWriter.EXPECT().WriteRange(gomock.Any()).Return(nil).
					DoAndReturn(func(info graveler.Range) error {
			...
			actualMetarangeID, err := sut.WriteMetaRange(context.Background(), tt.writeStorageID, graveler.StorageNamespace(ns), tt.records)
			if tt.errorIs != nil {
				require.ErrorIs(t, err, tt.errorIs)
			} else {
				require.NoError(t, err)
