...
func (c *CommitsMap) Set(id CommitID, node CommitNode) {
	c.Map[id] = node
}

// Get gets a commit.  If the commit has not been Set it uses CommitGetter
// to read it.
func (c *CommitsMap) Get(id CommitID) (CommitNode, error) {
	ret, ok := c.Map[id]
	if ok {
		return ret, nil
	}
...
// modify it.
func (c *CommitsMap) GetMap() map[CommitID]CommitNode {
	return c.Map
}

// nodeFromCommit returns a new CommitNode for a Commit.
func nodeFromCommit(commit *Commit) CommitNode {
	var mainParent CommitID
	if len(commit.Parents) > 0 {
		...
		if commit.Version < CommitVersionParentSwitch {
...
func GetGarbageCollectionCommits(ctx context.Context, startingPointIterator *GCStartingPointIterator, commitGetter RepositoryCommitGetter, rules *GarbageCollectionRules) (map[CommitID]MetaRangeID, error) {
	...
	processed := make(map[CommitID]time.Time)
	activeMap := make(map[CommitID]struct{})
...
func makeCommitMap(commitNodes map[CommitID]CommitNode, commitSet map[CommitID]struct{}) map[CommitID]MetaRangeID {
	res := make(map[CommitID]MetaRangeID)
